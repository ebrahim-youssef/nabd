...
| `wirdEntries`  | Primary key `id`; indexes `day`, `versionId`, compound `[day+itemId]`, and schema field `checkedAt` | Append-only check/uncheck events. Each event references the version in force for its day. Completion from checklist, counter, or adhkar uses the same event shape. Existing rows are never edited or deleted. The TypeScript row contains `at`; the schema indexes `checkedAt`. This mismatch must be resolved or deliberately preserved before porting queries. | Durable history. Currently local plus Supabase remote copy.                      |
