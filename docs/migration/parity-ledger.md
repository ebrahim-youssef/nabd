...
| Exact-alarm access unavailable             | Decided in [ADR-0016](../adr/0016-native-release-path.md): `USE_EXACT_ALARM` for Android 13+, `SCHEDULE_EXACT_ALARM` capped at `maxSdkVersion="32"` for Android 12. Android 13+ cannot lose the permission, so this state only arises on Android 12. | On Android 12, check `canScheduleExactAlarms()` and, when false, prompt with an actionable route to the system settings screen. Never degrade to inexact scheduling silently, and never report alarms as exact when they are not. |
...
- `android:allowBackup="false"` appears only in
  [the legacy manifest](../../android/app/src/main/AndroidManifest.xml), which arrived with the
  original Capacitor scaffold in NBD-46 (`0fbfb0a`) as a template default and leaves with that
  tree. `apps/native` has no `android/` directory: its manifest is generated by `expo prebuild`,
  so what it declares is a fresh choice expressed in `apps/native/app.json`, to be made with the
  publication work.
...
Until export and import ship and a restore is verified, migration implementations must not claim
that local persistence alone protects multi-year devotional history. Deferring the work does not
relax this; it is what keeps the claim from being made early.
