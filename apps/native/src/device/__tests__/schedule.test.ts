...
describe('buildPrayerSchedule', () => {
  it('builds a three-day future window from shared prayer times and moments', () => {
    ...
    expect(schedule.alarms.some((alarm) => alarm.channelKey === 'adhkarReminder')).toBe(true)
  ...
  it('excludes disabled moments and never emits stale alarms', () => {
    const schedule = buildPrayerSchedule({
      ...
      now: new Date('2026-09-20T23:59:59.000Z').getTime(),
    })

    expect(schedule.alarms.every((alarm) => alarm.channelKey === 'adhan' || alarm.channelKey === 'adhanFajr')).toBe(
      true,
    )
    expect(schedule.alarms.every((alarm) => alarm.at > new Date('2026-09-20T23:59:59.000Z').getTime())).toBe(
      true,
    )
  })
})
